...
/* ----------------------------------------------------------- */
/*  Naoya Takahashi:                                           */
/*             2015/10 Modified for Hybrid HMM ANN System      */
/* ----------------------------------------------------------- */
                       ...
                       int id, int idx)
{
   ...
   case SHAREDHS:
      /* x: MFCC*/
      v=x->fv[s]; /* always s=1 v[0]=(int)dim, v[1:(dim+1)]=vector */
      me=se->spdf.cpdf+1; /* MixtureElem */

    if (se->nMix==0){     /* Direct Case */
	     /* printf("id=%d, v[%d+1]=%f\n", id, idx, v[idx+1]); */
             bx=v[idx+1];  /* Direct Copy */
    }else if (se->nMix==1){     /* Single Mixture Case */
         
         if (me->mpdf->mIdx>0 && me->mpdf->mIdx<=pri->psi->nmp){
			printf("me->mpdf->mIdx %d, pri->psi->nmp\n", me->mpdf->mIdx, pri->psi->nmp);
            pre=pri->psi->mPre+me->mpdf->mIdx;
		 }else pre=NULL;

         if (pre==NULL) {
			 /*  MOutP(Log prob of x, MixPDF), id:Time */
			/* printf("pre==NULL, id=%d\n", id); */
...
static LogFloat cPOutP(PSetInfo *psi,Observation *obs,StateInfo *si,int id, int idx)
{
   PreComp *pre;
   LogFloat outp;
   StreamElem *se;
   ...
   if (pre->id!=id) { /* bodged at the moment - fix !! */
      if ((FALSE && psi->mixShared==FALSE) || (psi->hset->hsKind == DISCRETEHS)) {
         ...
         S=obs->swidth[0]; /* [0]=num streams,[i]=width of stream i */
         if (S==1 && si->weights==NULL){
		    /* printf("S=1, si->weights==NULL, id = %d\n", id); */
            outp=cSOutP(psi->hset,1,obs,si->pdf+1,id, idx); /* (HMM set, int, Observation, StreamElem) */ 
         }
         else {
            outp=0.0;
            se=si->pdf+1;
			...
			printf("S=%d, id = %d\n", S,id);
            for (s=1;s<=S;s++,se++){
               outp+=w[s]*cSOutP(psi->hset,s,obs,se,id, idx); /* (HMM set, int, Observation, StreamElem) */ 
            }
         }
      }
      pre->outp=outp;
...
static void StepHMM1(NetNode *node) /* Model internal propagation NBEST */
{
   ...
   hmm=node->info.hmm; 
   N=hmm->numStates; /* N=5 start, 3states, end */
   trP=hmm->transP;
   seIndex=pri->psi->seIndexes[hmm->tIdx];

#ifdef DEBUG_PRINT   
   printf("StepHMM1n N=%d, hmm->tIdx=%d\n",N, hmm->tIdx);
#endif
   for (j=2,res=pri->psi->sBuf+2;j<N;j++,res++) {  /* Emitting states first */
      ...
      if (res->tok.like > pri->genThresh) { /* State pruning */
		 /* printf("j=%d\n", j);*/ /* j:state number 2,3,4, Should I accsess to (j-2)+(N-2)*(hmm->tIdx -1)? */
		 /* Time:id, Phone(word)index:hmm->tIdx-1, StateNo.:j-2 */ 
		 PPidx = (j-2)+ (hmm->tIdx-1)*(N-2);
		 /* printf("PPidx=%d\n", PPidx);*/
         outp=cPOutP(pri->psi,pri->obs,hmm->svec[j].info,pri->id, PPidx); /* Observation Prob, cPOutP(PSetInfo,Observation,StateInfo,id) */
         res->tok.like+=outp;  /* Likelihood of token */
...
void ProcessObservation(VRecInfo *vri,Observation *obs,int id, AdaptXForm *xform)
{
   ...
   for (inst=pri->head.link,j=0;inst!=NULL;inst=inst->link,j++){
      if (inst->node)
         StepInst1(inst->node); /* inst->node: hmm */
   }
