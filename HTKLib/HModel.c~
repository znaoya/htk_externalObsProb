                             ...
                             int M, int s, Vector tpdf)
{
   ...
   printf("GetTiedMixtures: hset->tmRecs[s].mixId=%s\n", hset->tmRecs[s].mixId->name);
   if (isNew) {
...
{
   unsigned hashval;
   /* HMM\96\BC\82ɉ\9E\82\B6\82\C4hashval\82\AA\8C\88\82܂\E9-> \8F\87\94Ԃ\CDhmmsdef.hmm\8F\87\82ł͂Ȃ\A2 */
...
MLink FindMacroName(HMMSet *hset, char type, LabId id)
{
   ...
   hashval = Hash(id->name);
#ifdef DEBUG_PRINT
   printf("FindMacroName:hashval=%d, id->name=%s \n",hashval, id->name);
#endif
   m = hset->mtab[hashval];
...
void SetIndexes(HMMSet *hset)
{
   ...
   do {
	  ...
	  printf("hss.hmm->tIdx=%d, hss.mac->id->name=%s\n", hss.hmm->tIdx, hss.mac->id->name);
   }
   while(GoNextHMM(&hss));
   EndHMMScan(&hss);
   NewHMMScan(hset,&hss);
...
ReturnStatus LoadHMMSet(HMMSet *hset, char *hmmDir, char *hmmExt)
{
   ...
   for (h=0; h<MACHASHSIZE; h++)
      for (p=hset->mtab[h]; p!=NULL; p=p->next) 
         if (p->type == 'h'){
			/* printf("LoadHMMSet:h=%d, p->type=%c, p->id->name=%s\n",h, p->type, p->id->name); */ /* NAOYA id-> name = one, two ,three! */
...
static ReturnStatus InitHMMSet(HMMSet *hset, char *fname, Boolean isSingle)
{
   ...
   if (isSingle){
      ...
      while(ReadString(&src,buf)) {
         lId = GetLabId(buf,TRUE); /* contents of buf list, one two..., lId 4296240 ...*/
#ifdef DEBUG_PRINT
		 printf("buf:%s, lId=%d\n", buf,lId);
#endif
         SkipWhiteSpace(&src);
...
LogFloat MOutP(Vector x, MixPDF *mp)
{
   ...
   vSize=VectorSize(x); /* = 39 (dim of feature) */
   switch (mp->ckind) {
